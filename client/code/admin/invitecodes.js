...
function setupInviteCodes () {
  ...
  $body.on('click', '#invite-button', function (event) {
    ...
    ss.rpc('admin.invitecodes.newGameInstance', newGameData, function (err, res) {
      if (err) {
        message = 'Cannot create this game: An error occured with the database. Please notify the administrator.'
        $message.addClass('error').text(message)
        $button.addClass('btn-error')
        console.log('error with db', err)
      } else if (res) {
        message = 'Cannot create this game: Game name already exists.'
        $message.addClass('error').text(message)
        $button.addClass('btn-error')
      } else {
        // Send invites
        ss.rpc('admin.invitecodes.sendInvites', emailList, instanceName, function (res) {
          ...
          message = 'Game has been created and invites have been sent!'
